...
from utils import pkl_filename, raw_dataset_path
...
def load_dataset(raw_dataset=raw_dataset_path, fixed_baseline=True):
    """
    load ECG from utils.pkl_filename. If fixed_baseline=True - remove baseline wander
    since the BW is removed for a long time, the processing result is stored in raw_dataset.
    If file exist in raw_dataset, loads dataset from it

    :return: dictionary {"x":X, "y":Y}
    """
